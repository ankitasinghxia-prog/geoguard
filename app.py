...
def create_risk_map_image(lat, lon, risk_score, features=None):
    """Create a risk map image using matplotlib"""
    try:
        ...
        ax.plot(lon, lat, 'o', markersize=20, color=risk_color, 
                markeredgecolor='white', markeredgewidth=2, zorder=10)
        
        circle = plt.Circle((lon, lat), 0.02, color=risk_color, alpha=0.3, zorder=5)
        ax.add_patch(circle)
        
        ax.annotate(f'{risk_score:.1f}%', xy=(lon, lat), xytext=(10, 10),
                   textcoords='offset points', fontsize=16, fontweight='bold',
                   color=risk_color, bbox=dict(boxstyle='round,pad=0.3', 
                                               facecolor='white', alpha=0.9))
        
        ax.set_title(f'GeoGuard Risk Analysis', fontsize=18, fontweight='bold', pad=20)
        
        info_text = f'📍 Location: {lat:.4f}°N, {lon:.4f}°E\n'
        info_text += f'📊 Risk Score: {risk_score:.1f}%\n'
        info_text += f'🎯 Risk Level: {risk_text}'
        ...
        ax.text(0.5, 0.02, 'Generated by GeoGuard AI - Border Terrain Risk Analyzer',
               transform=ax.transAxes, fontsize=8, ha='center', color='#888888')
...
with st.sidebar:
    ...
    st.caption("💡 Copy the link above to share this location")
    ...
    if st.button("📷 Save Map as Image", use_container_width=True):
        if st.session_state.last_risk_score is not None:
            with st.spinner("Generating image..."):
                ...
                img_bytes = create_risk_map_image(
            ...
            st.warning("⚠️ Please analyze a location first")
